...
#
# Why this is needed:
# - langgraph-checkpoint-cloudflare-d1 depends on langgraph>=1.0.0
...
#
# Adapted from libs/langchain-cloudflare/examples/workers/scripts/setup_pyodide_deps.sh.
# This checkpointer only needs langgraph.checkpoint / langgraph.graph, not
# langgraph.prebuilt, so unlike that script this one does not extract the
# separate langgraph-prebuilt wheel.
